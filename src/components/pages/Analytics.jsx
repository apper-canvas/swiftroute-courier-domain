...
const volumeChartOptions = (volumes = []) => ({
...
labels: delays.map(d => d?.reason || 'Unknown'),
    legend: { position: "bottom" },
    dataLabels: { enabled: true, formatter: (val) => `${val.toFixed(1)}%` }
  };

const delayChartSeries = (delays = []) => delays.map(d => d?.count || 0);
...
dataLabels: { enabled: false },
    xaxis: { categories: hourly.map(h => `${h?.hour || 0}:00`), labels: { style: { colors: "#475569" } } },
    yaxis: { labels: { style: { colors: "#475569" } } },
    tooltip: { theme: "light" }
  };
const hourlyChartSeries = (hourly = []) => [
    { name: "Deliveries", data: hourly.map(h => h?.deliveries || 0) }
  ];
...
<motion.div key="volume-chart" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <Card className="p-6">
            ...
            {volumes.length > 0 ? (
              <ReactApexChart
                options={volumeChartOptions(volumes)}
                series={volumeChartSeries(volumes)}
                type="area"
                height={300}
              />
            ) : (
              <Empty message="No volume data available" />
            )}
          </Card>
        </motion.div>
          ...
          <Card className="p-6">
            ...
            {delays.length > 0 ? (
              <ReactApexChart
                ...
                series={delayChartSeries(delays)}
                type="pie"
                height={300}
              />
            ) : (
              <Empty message="No delay data available" />
            )}
          </Card>
        </motion.div>
      </div>
        ...
        <Card className="p-6">
          ...
          {hourly.length > 0 ? (
            <ReactApexChart
              ...
              series={hourlyChartSeries(hourly)}
              type="bar"
              height={300}
            />
          ) : (
            <Empty message="No hourly distribution data available" />
          )}
        </Card>
      </motion.div>
{/* Driver Performance */}
<motion.div key="driver-performance" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
        <Card className="p-6">
          ...
          {performance.length > 0 ? (
                ...
                <tbody>
                  {performance.map((driver, index) => (
                    ...
                    >
                      ...
                      <td className="py-3 px-4 font-medium text-secondary">{driver?.name || 'Unknown'}</td>
                      <td className="py-3 px-4 text-right text-secondary">{driver?.deliveries || 0}</td>
                      <td className="py-3 px-4 text-right">
                        <span className="font-semibold text-success">{driver?.onTime || 0}%</span>
                      </td>
                      <td className="py-3 px-4 text-right">
                        <span className="font-semibold text-secondary">{driver?.rating || 0}</span>
                      </td>
            ...
            <Empty message="No driver performance data available" />
          )}
        </Card>
      </motion.div>
    </div>
